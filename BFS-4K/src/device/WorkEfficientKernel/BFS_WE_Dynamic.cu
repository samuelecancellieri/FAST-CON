...
__global__ void DynamicKernel( 	int* __restrict__ devEdge,
	...
	while (flag) {
		while (k < end && founds < REG_QUEUE) {
			...
			KVisit<BlockDim, false>(dest, devDistance, Queue, founds, level, NULL);
	...
	if (DYNAMIC_PARALLELISM) {
		...
		if (degree >= THRESHOLD_G) {
			if (SINGLE || (Tid & _Mod2<WARP_SZ>::VALUE) == 0) {
				...
				DynamicKernel<BlockDimDyn><<< LAUNCHED_BLOCKS, BlockDimDyn, SMem_Per_Block(BlockDimDyn) >>> (devEdge, devDistance, devF2, start, end, level);
			}
			end = INT_MIN;
		}
	}
