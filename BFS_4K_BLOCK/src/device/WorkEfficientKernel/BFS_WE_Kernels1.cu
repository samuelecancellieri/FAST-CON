...
__device__ __forceinline__ void KVisit(const int dest, dist_t *devDistance,
									   int *Queue, int &founds, const int level, volatile long long int *HashTable, const int index, bool *adjMatrix)
{
#if ATOMICCAS
	if (atomicCAS((int *)&devDistance[dest], (int)(dist_t)-1, (int)devDistance[index]) == (dist_t)-1)
	{
		Queue[founds++] = dest;
	}
	else if (atomicCAS((int *)&devDistance[dest], (int)devDistance[index], (int)devDistance[index]) != devDistance[index])
	{
		adjMatrix[(devDistance[dest] * NUM_SOURCES) + devDistance[index]] = 1;
		adjMatrix[(devDistance[index] * NUM_SOURCES) + devDistance[dest]] = 1;
	}
...
__device__ __forceinline__ void EdgeVisit(int *__restrict__ devEdge,
										  ...
										  int start, int end,
										  int *Queue, int &founds, const int level,
										  volatile long long int *HashTable, const int index, bool *adjMatrix)
{
	...
	for (int k = start + (Tid & _Mod2<WARP_SZ>::VALUE); k < end; k += WARP_SZ)
	{
		...
		KVisit<BlockDim, DUP_REM>(dest, devDistance, Queue, founds, level, HashTable, index, adjMatrix);
	}
#elif SAFE == 1
	bool flag = true;
