...
__GLOBAL_DEVICE__ void NAME1(int *__restrict__ devNode,
							 ...
							 const int devF1Size, const int level, bool *devAdjMatrix)
{
	...
	for (int t = VirtualID; t < devF1Size; t += Stride)
	{
		...
		EdgeVisit<BlockDim, WARP_SZ, DUP_REM>(devEdge, devDistance, devF2, devF2SizePrt, start, end, Queue, founds, level, HashTable, index, devAdjMatrix);
	}
#else
	...
	for (int t = VirtualID, loop = 0; loop < maxLoop; t += Stride, loop++)
	{
		...
		if (t < devF1Size)
		{
			...
			DynamicParallelism<BlockDim, WARP_SZ, 0>(devEdge, devDistance, devF2, start, end, level, index, devAdjMatrix);
		}
		else
			end = INT_MIN;
							  ...
							  const int devF1Size, const int level, bool *devAdjMatrix)
{
	...
	for (int ID = blockIdx.x * BlockDim + Tid; ID < size; ID += Stride)
	{
		...
		if (ID < devF1Size)
		{
			...
			DynamicParallelism<BlockDim, WARP_SZ, 1>(devEdge, devDistance, devF2, Tstart, Tend, level, index, devAdjMatrix);
		}
		else
			Tend = INT_MIN;
