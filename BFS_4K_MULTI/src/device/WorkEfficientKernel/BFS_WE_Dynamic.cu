...
__global__ void DynamicKernel(int *__restrict__ devEdge,
							  ...
							  const int level, const int index, bool *devAdjMatrix)
{
	...
	while (flag)
	{
		while (k < end && founds < REG_QUEUE)
		{
			...
			KVisit<BlockDim, false>(dest, devDistance, Queue, founds, level, NULL, index, devAdjMatrix);
	...
	if (DYNAMIC_PARALLELISM)
	{
		...
		if (degree >= THRESHOLD_G)
		{
			if (SINGLE || (Tid & _Mod2<WARP_SZ>::VALUE) == 0)
			{
				...
				DynamicKernel<BlockDimDyn><<<LAUNCHED_BLOCKS, BlockDimDyn, SMem_Per_Block(BlockDimDyn)>>>(devEdge, devDistance, devF2, start, end, level, index, devAdjMatrix);
			}
			end = INT_MIN;
		}
	}
